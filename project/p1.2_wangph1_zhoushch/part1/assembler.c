    ...
    while (fgets(buffer, BUF_SIZE, input))
    {
        ...
        if (data_seg)
        {
            ...
            while ((prostr = strtok(NULL, IGNORE_CHARS)))
            {
                ...
                /* get the needed bits */
                tra_num &= USNG_UPPER_BOUND(data_bytes * 8);
                /* add bytes */
                data_buffer = data_buffer + (tra_num << (data_buffer_track * 8));
                /* update track */
                data_buffer_track += data_bytes;
                /* if buffer is full */
...
int pass_two(FILE *original, FILE *symtbl, FILE *basic, FILE *machine) {
    ...
    while (fgets(buffer, BUF_SIZE, original))
    {
        ...
        if (!strcmp(buffer, "-"))
        {
            /* if buffer is "-" then continue */
            continue;
        }
        /* if buffer is empty then continue */
        if (strlen(buffer) == 0)
        {
            /* if buffer is empty then continue */
            continue;
        ...
        /* if num_args > MAX_ARGS then raise error */
        if (num_args == 3 && (!strcmp(instruction_name, "lb") || !strcmp(instruction_name, "lh") || !strcmp(instruction_name, "lw") || !strcmp(instruction_name, "lbu") || !strcmp(instruction_name, "lhu")))
        {
            /*args*/
            char *tmp = arg_list[1];
